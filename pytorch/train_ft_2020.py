...
if not opt.resume:
    ...
    if opt.use_dense:
        ...
        model = ft_net_SE(len(class_names), opt.droprate, opt.stride, opt.pool, None)
    elif opt.use_DSE:
...
if opt.init_name != 'imagenet':
    ...
    opt.use_SE = old_opt.use_SE
    if opt.use_dense:
        model = ft_net_dense(opt.nclasses, droprate=opt.droprate, stride=opt.stride, init_model=init_model, pool = opt.pool)
    elif opt.use_SE:
        print('yes')
        model = ft_net_SE(opt.nclasses, droprate=opt.droprate, stride=opt.stride, pool = opt.pool, init_model = init_model)
    else:
...
exp_lr_scheduler = lr_scheduler.MultiStepLR(optimizer_ft, milestones=[30-start_epoch], gamma=0.1)
...
model = train_model(model, criterion, optimizer_ft, exp_lr_scheduler,
                    start_epoch=start_epoch, num_epochs=40)
