...
save_path = 'data/pytorch-cam'
save_query_path = 'data/pytorch-cam/query/'
save_train_path = 'data/pytorch-cam/train/' # 233 classes
save_train_all_path = 'data/pytorch-cam/train_all/' # include train and val
save_test_path = 'data/pytorch-cam/gallery'

save_test_data_path = 'data/test_data-cam'
save_test_query_path = 'data/test_data-cam/query'
save_test_gallery_path = 'data/test_data-cam/gallery'
...
for s in itemlist:
    ...
    dst_path = save_train_all_path + cam  #train_all
    ...
    if int(vid) <= 400:  # ID smaller than 400 will used as train
        dst_path = save_train_path + cam
        dst_path_val = save_query_path + cam
        if not os.path.isdir(dst_path):
            os.mkdir(dst_path)
        if not os.path.isdir(dst_path_val):
            os.mkdir(dst_path_val)
        copyfile(src_path, dst_path + '/'+vid + '_' + cam + '_'+ name)
